...
RegisterNetEvent('primordial_admin:server:checkNotPassed', function(source, feature)
    ...
    PL.Print.Error(('^3Player ^1%s ^3tried to use the ^5%s ^3feature without permission.'):format(sourrceOfCheckFailed['name'], feature))
end)
...
RegisterNetEvent('primordial_admin:server:teleportToPlayer', function(targetId)
    ...
    if not targetCoords then
        return PL.Print.Error(('Staff member try to teleport to a player but the coords of the player are not found.'))
    end
...
RegisterNetEvent('primordial_admin:server:teleportPlayerTo', function(targetId, targetName)
    ...
    if not sourceCoords then
        return PL.Print.Error(('Staff member try to teleport a player to him but his coords are not found.'))
    end
...
RegisterNetEvent('primordial_admin:server:teleportBackPlayer', function(targetId)
    ...
    if not targetCoords then
        return PL.Print.Error(('Staff member try to teleport a player back to his previous location but the coords are not found.'))
    end
...
PL.RegisterCommand('pl', {'admin'}, function(sPlayer)
    PL.Print.Debug(json.encode(sPlayer, { indent = true }))
end, false, {
    help = 'Player',
})