...
lib.callback.register("primordial:server:createSocietyDB", function(societyOwnerId, societyName, societyLabel, isWhitelisted)
    ...
    if not ownerIdentifier then
        PL.Print.Log(3, "Failed to retrieve owner identifier for society creation.")
        return false
    end
    ...
    if not queryResult then
        PL.Print.Log(3, "Failed to insert society into the database.")
        return false
    end

    PL.Print.Log(1, "Society created successfully with owner license: " .. ownerIdentifier.license .. " and registration number: " .. registrationNumber)
    return true
end)
...
lib.callback.register("primordial:server:transferSocietyOwner", function(source, societyName, newOwnerId)
    ...
    if not society then
        PL.Print.Log(3, ("Society '%s' does not exist. Transfer aborted."):format(societyName))
        return false
    end

    local currentPlayer <const> = PL.GetPlayerFromId(source)
    if not currentPlayer or currentPlayer.identifier ~= society.owner then
        PL.Print.Log(3, ("Player '%s' is not the current owner of society '%s'. Transfer aborted."):format(source, societyName))
        return false
    end

    local newOwner <const> = PL.GetPlayerFromId(newOwnerId)
    if not newOwner then
        PL.Print.Log(3, ("New owner with ID '%s' does not exist. Transfer aborted."):format(newOwnerId))
        return false
    end
    ...
    if not result then
        PL.Print.Log(3, ("Failed to update owner for society '%s'. Transfer aborted."):format(societyName))
        return false
    end

    PL.Print.Log(1, ("Ownership of society '%s' has been transferred to '%s'."):format(societyName, newOwner.identifier))
    return true
end)