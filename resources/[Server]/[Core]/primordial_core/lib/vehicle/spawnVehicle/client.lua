...
function PL.Vehicle.SpawnClientVehicle(vehicleModel, coords, heading, cb, networked)
    ...
    if dist > 424 then -- Onesync infinity Range (https://docs.fivem.net/docs/scripting-reference/onesync/)
        ...
        return PL.Print.Error(("Resource ^1%s^5 Tried to spawn vehicle on the client but the position is too far away (Out of onesync range)."):format(executingResource))
    end
