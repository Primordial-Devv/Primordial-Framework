...
local function registerSociety(name, label, data)
    if Societies[name] then
        PL.Print.Warning(('Society already registered, name: ^5%s^7'):format(name))
        return
    end
...
AddEventHandler('primordial_core:server:withdrawMoney', function(societyName, amount)
    ...
    if not society then
        PL.Print.Warning(('Player ^5%s^7 attempted to withdraw from non-existing society - ^5%s^7!'):format(source, societyName))
        return
    end
...
AddEventHandler('primordial_core:server:depositMoney', function(societyName, amount)
    ...
    if not society then
        PL.Print.Warning(('Player ^5%s^7 attempted to deposit to non-existing society - ^5%s^7!'):format(source, societyName))
        return
    end
