...
local function CreateSocietyMenu(societyName)
    ...
    if #nearbyPlayers == 0 then
        PL.Print.Error('No nearby players for ownership transfer.')
        return
    end
    ...
    PL.Print.Debug('isWhitelisted: ' .. isWhitelisted)
    ...
    if not success then
        PL.Print.Error('An error occurred while creating the society.')
        return
    end

    PL.Print.Info('The society has been created successfully and the owner is ' .. selectedPlayerName)
end
