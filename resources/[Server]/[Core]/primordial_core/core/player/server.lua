    ...
    MySQL.prepare(
        ...
        function(affectedRows)
            if affectedRows == 1 then
                PL.Print.Log(1, ('Saved player ^5"%s^7"'):format(sPlayer.name))
            end
            if cb then
                cb()
            end
...
function PL.SavePlayers(cb)
    ...
    MySQL.prepare(
        ...
        function(results)
            ...
            PL.Print.Log(1, ("Saved ^5%s^7 %s over ^5%s^7 ms"):format(#parameters, #parameters > 1 and "players" or "player", PL.Math.Round((os.time() - startTime) / 1000000, 2)))
        end
    )
end
...
function CreatePrimordialPlayer(identifier, playerId, data)
    ...
    if PL.AdminPermissions(playerId) then
        PL.Print.Log(1, ("Player ^5%s^0 Has been granted admin permissions via ^5Ace Perms^7."):format(playerId))
        defaultGroup = "admin"
    end
...
function LoadPrimordialPlayer(identifier, playerId, isNew)
    ...
    if not PL.DoesSocietyExist(job, grade) then
        PL.Print.Log(2, ("Ignoring invalid job for ^5%s^7 [job: ^5%s^7, grade: ^5%s^7]"):format(identifier, job, grade))
    ...
    PL.Print.Log(1, ('Player ^5"%s"^0 has connected to the server. ID: ^5%s^7'):format(sPlayer.getName(), playerId))
end